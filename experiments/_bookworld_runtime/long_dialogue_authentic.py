"""
Long Dialogue Benchmark Experiment (BookWorld-runtime variant).

NOTE: This variant drives the long-dialogue benchmark through the BookWorld
simulation runtime (``Server`` -> ``Performer.single_role_interact``). That
runtime (``server.py`` and the full ``Performer`` integration) is NOT shipped
in this repository — it belongs to the upstream BookWorld product. It is kept
here only as a reference for how PersonaForge plugged into a multi-agent world.

For a SELF-CONTAINED, dependency-free long-dialogue benchmark (only an API key
needed), use ``experiments/sft/long_dialogue_4way.py`` instead.

Measures: PC trajectory over 50 turns, dual-process trigger rate, drift, recovery.
"""
...
try:
    from personaforge.core.server import Server  # provided by the upstream BookWorld runtime (not shipped)
except ImportError as _e:  # pragma: no cover
    raise ImportError(
        "This script needs the BookWorld simulation runtime (personaforge.core.server.Server), "
        "which is not included in this repository. Use experiments/sft/long_dialogue_4way.py for "
        "the self-contained long-dialogue benchmark."
    ) from _e
from personaforge.utils import load_json_file, save_json_file
from personaforge.llm.Gemini import Gemini
