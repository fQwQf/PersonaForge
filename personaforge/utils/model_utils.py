...
def get_models(model_name: str):
    ...
    if os.getenv("OPENROUTER_API_KEY", default="") and model_name in MODEL_NAME_DICT:
        from personaforge.llm.OpenRouter import OpenRouter
        ...
        from personaforge.llm.LangChainGPT2 import LangChainGPT
        ...
        from personaforge.llm.Claude import Claude
        ...
        from personaforge.llm.Qwen import Qwen
        return Qwen(model=model_name)
    elif model_name.startswith('deepseek'):
        from personaforge.llm.DeepSeek import DeepSeek
        return DeepSeek(model=model_name)
    elif model_name.startswith('doubao'):
        from personaforge.llm.Doubao import Doubao
        return Doubao()
        ...
        print(f"[model_utils] 模型选择: {model_name}, is_new_model={is_new_model}, should_use_vertex={should_use_vertex}, has_google_creds={has_google_creds}")
        
        if should_use_vertex:
            try:
                from personaforge.llm.VertexGemini2 import VertexGemini
        ...
        from personaforge.llm.LangChainGPT import LangChainGPT
        return LangChainGPT()
...
def build_db(data, db_name, db_type, embedding, save_type="persistent"):
    ...
    if not data or not db_name:
        return None
    from personaforge.db.ChromaDB import ChromaDB
